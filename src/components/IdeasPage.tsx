...
export function IdeasPage({ onNavigateToResearch }: IdeasPageProps) {
  ...
  const { data: ideas = [], isLoading, isError, refetch } = useTradeIdeas({
    type: filterType,
    source: filterSource,
    status: filterStatus,
    search: searchQuery,
  ...
  const bullishCount = ideas.filter((i) => i.type === 'BULLISH').length;
  const bearishCount = ideas.filter((i) => i.type === 'BEARISH').length;
  const aiCount = ideas.filter((i) => i.source === 'AI_AGENT').length;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-3 duration-500 pb-24">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Document proprietary theses or summon autonomous AI agents to discover high-conviction trade setups.
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="bg-card/50 backdrop-blur-md border-border/60 hover:border-primary/30 transition-all">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-black font-mono text-foreground">{ideas.length}</p>
            </div>
      ...
      <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between bg-card/60 backdrop-blur-md p-3.5 rounded-2xl border border-border/60 shadow-sm">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            placeholder="Search by symbol, catalyst, thesis, or #tag..."
            value={searchQuery}
        ...
        <Card className="p-12 text-center bg-card/30 border-dashed border-2 border-border/60">
          ...
          <p className="text-sm text-muted-foreground max-w-md mx-auto mb-6">
            {searchQuery || filterType !== 'ALL' || filterSource !== 'ALL'
              ? 'No trade ideas matched your active filters. Try broadening your search.'
            ...
            return (
              ...
              >
                ...
                <CardContent className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                  {/* Card Header: Symbol, Sentiment & Source Badge */}
                  <div>
                  ...
                  <div className="space-y-3 pt-2 border-t border-border/40">
                    ...
                    {/* Meta: Source, R:R, Status, and Tags */}
